...
export default {
  ...
  methods: {
	...
	this.ruleForm.shebeixiangqing = this.ruleForm.shebeixiangqing.replace(reg,'../../../ssm88j08/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `chufangshebei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.chufangshebeiCrossAddOrUpdateFlag = false;
                  this.parent.search();
